...
import { Plus, TrendingUp, AlertCircle, CheckCircle2, AlertTriangle, X } from 'lucide-react';
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <header className="flex justify-between items-end">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-mint text-white px-5 py-2.5 rounded-lg font-medium hover:bg-deepmint transition-colors shadow-sm shadow-mint/20 flex items-center gap-2">
        ...
        {loading ? (
          ...
          <>
            ...
            {budgets.map(b => {
              ...
              return (
                <div key={b.id} className={`bg-white p-6 rounded-2xl border shadow-sm hover:shadow-md transition-shadow cursor-pointer ${isWarning ? 'border-coral/30' : 'border-ink/5'}`}>
                  <div className="flex justify-between items-start mb-6">
                    ...
                    {isWarning ? (
                      <span className="bg-coral/10 text-coral px-2.5 py-1 rounded-md text-xs font-bold flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Warning</span>
                    ) : isAttention ? (
                      <span className="bg-amber/10 text-amber px-2.5 py-1 rounded-md text-xs font-bold flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Attention</span>
                    ) : (
                      <span className="bg-mint/10 text-mint px-2.5 py-1 rounded-md text-xs font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Normal</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-ink/50">{formatCurrency(b.amount || 0)}</span>
                    </div>
                  ...
                  <div className={`text-xs flex items-center gap-1 ${isWarning ? 'text-coral font-medium' : isAttention ? 'text-amber-900/70' : 'text-ink/50'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-ink/5">
            <div className="flex justify-between items-center p-6 border-b border-ink/5">
              <h2 className="font-display text-xl font-bold">Create Budget</h2>
            ...
            <form onSubmit={handleCreateBudget} className="p-6 space-y-5">
              ...
              <div className="pt-2">
                ...
                >
                  {isSubmitting ? 'Creating...' : 'Save Budget'}
                </button>
              </div>
            </form>
          </div>
